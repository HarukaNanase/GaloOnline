...
SuccessMessage = "ACK"
ErrorMessage = "ERR"
ReadingSocket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
ReadingSocket.bind(("127.0.0.1",1337))
...
Inputs = [ClientSocket, ReadingSocket]
...
while(True):
    ...
    for i in ins:
        if i == ReadingSocket:
            Console = True
            command = ReadFromSocket(ReadingSocket)
        elif i == ClientSocket:
            Socket = True
            command = ReadFromSocket(ClientSocket)
